...
import { APIPromise } from 'vra_iaas/core/api-promise';

import util from 'node:util';
import VraIaas from 'vra_iaas';
import { APIUserAbortError } from 'vra_iaas';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['VRA_IAAS_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: VraIaas) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new VraIaas({ bearerToken: 'My Bearer Token' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['VRA_IAAS_LOG'] = 'debug';
      const client = new VraIaas({ logger: logger, bearerToken: 'My Bearer Token' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['VRA_IAAS_LOG'] = 'not a log level';
      const client = new VraIaas({ logger: logger, bearerToken: 'My Bearer Token' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'VRA_IAAS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['VRA_IAAS_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new VraIaas({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['VRA_IAAS_BASE_URL'] = 'https://example.com/from_env';
      const client = new VraIaas({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['VRA_IAAS_BASE_URL'] = ''; // empty
      const client = new VraIaas({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://vra1.vcf.lab');
    });

    test('blank env variable', () => {
      process.env['VRA_IAAS_BASE_URL'] = '  '; // blank
      const client = new VraIaas({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://vra1.vcf.lab');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new VraIaas({ bearerToken: 'My Bearer Token', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['VRA_IAAS_BASE_URL'] = 'http://localhost:5000/env';
      const client = new VraIaas({ bearerToken: 'My Bearer Token' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new VraIaas({ maxRetries: 4, bearerToken: 'My Bearer Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new VraIaas({ bearerToken: 'My Bearer Token' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['VRA_IAAS_BEARER_TOKEN'] = 'My Bearer Token';
    const client = new VraIaas();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['VRA_IAAS_BEARER_TOKEN'] = 'another My Bearer Token';
    const client = new VraIaas({ bearerToken: 'My Bearer Token' });
    expect(client.bearerToken).toBe('My Bearer Token');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new VraIaas({ bearerToken: 'My Bearer Token', fetch: testFetch });
